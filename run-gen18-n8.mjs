#!/usr/bin/env node
/**
 * run-gen18-n8.mjs — gen18_evolved k=5 control on N=8 problems.
...
const RUN_DIR = join('validation-runs', `gen18-evolved-n8-k5-${TIMESTAMP}`);
const TRACE_DIR = join(RUN_DIR, 'traces');

mkdirSync(TRACE_DIR, { recursive: true });

console.log(`\n${'='.repeat(60)}`);
console.log(`=== ${BASELINE} N=8 k=5 Control ===`);
...
for (const problem of PROBLEMS) {
  ...
  for (let trial = 0; trial < K; trial++) {
    ...
    try {
      ...
      trialResults.push({
        ...
        stageFailed: entries[0]?.[1]?.stageFailed || null,
      });

      const p1 = passAt1 ? '✓' : '✗';
      console.log(`    pass@1: ${p1}  pass@N: ${passAtN ? '✓' : '✗'}  attempts: ${entries.length}`);
    } catch (err) {
  ...
  results[problem] = {
    ...
    passAtN: anyPass ? `${K}/${K}` : `${trialResults.filter(t => t.passAtN).length}/${K}`,
    trials: trialResults,
  };
